  ...
  return (
    <div className="wrapper-socialBtns">
     ...
     <div onClick={() => saveToGTM('whatsapp',props.data.business_name)}> 
       <WhatsappShareButton 
        url={`whatsapp://send?text=חיפשתי ומצאתי ע"י המדריך החרדי WWW.402.co.il ${(props.data.business_name || (props.data.first_name + ' ' + props.data.last_name))} ${(props.data.link ? (location.origin + props.data.link) : location.href)}`}
        title={'חיפשתי ומצאתי ע"י המדריך החרדי WWW.402.co.il'}
      >
        <WhatsappIcon
          size={32}  
