      ...
      <div className="main-search full-height-container search">
        ...
        {this.props.results.length > 0 ? <div className={`wrapper-results ${this.state.showSideBarNearMe ? 'geo' : ''}`}>
          ...
          <Results
            ...
            lat={this.lat}
            lon={this.lon}
          /></div> : !this.props.loading ? <NoResults data={{ query: this.props.query }} /> : <span>Loading...</span>}
        {/*<Snackbar
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    handleResultsPageClick: (offset, captcha) => {
      ...
      dispatch(loadResultsAction({ page: offset, captcha, location }));
    },
    ...
    handleGetNearMe: (points) => {
      ...
      dispatch(loadResultsAction({ page: 1, location }));
    },
